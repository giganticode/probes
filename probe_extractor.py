    ...
    with open(json_features, "w") as writer:
        with torch.no_grad():
            for input_ids, input_mask, example_indices in eval_dataloader:
                ...
                all_outputs = model(input_ids=input_ids)#, attention_mask=input_mask) # token_type_ids=None,
            ...
            for model_checkpoint in list(model_checkpoints.keys()):
                for label_count in label_counts:
                    ...
                    if model_checkpoint == "BERT":
                        ...
                        model     = BartModel.from_pretrained(modelname, config=config, cache_dir="/tmp") 
